function samples = jitter_color( X , n , M0 , M1 , p , N_colors )
...
for s=1:n
    k  = sampled(s) ;
    position = mod( k-1 , size(occupied,1) ) + 1 ;
    ...
    if occupied(position) % 
        ...
        for i=1:ni
            if inds(i) ~= position
                ...
                samples{ns}.forward_prob   = p(position)/ni ;
            else
                for j=1:N_colors
                    ns = ns+1 ;
                    samples{ns}.flips = position + (j-1)*M0*M1 ;
                    samples{ns}.forward_prob = p(position)/ni/N_colors ;
                end
            end
        end
    else
        for j=1:N_colors
            ns = ns+1 ;
            samples{ns}.flips = position + (j-1)*M0*M1 ;
            samples{ns}.forward_prob  = p(position)/N_colors ;
        end
    end
end
...
%     position = mod( k-1 , size(occupied,1) ) + 1 ;
%     [i,j]    = ind2sub( [M0 M1] , position ) ;
%     color    = floor((k-1) / size(occupied,1)) + 1 ;
